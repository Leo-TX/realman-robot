...
def test_process_2d_mask(mask):
    ...
    def get_mask_center(mask):
        # Convert mask tensor to numpy array
        mask_np = mask.detach().cpu().numpy().astype('uint8') * 255
        # Convert mask array to binary image
        binary_image = mask_np.reshape(mask_np.shape[0], mask_np.shape[1], 1)
        # Calculate moments
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            # print(f'predictions["instances"]:{predictions["instances"]}')
            # print(f'size predictions["instances"]:{predictions["instances"].pred_masks[0].size()}')
            # print(f'type predictions["instances"]:{type(predictions["instances"].pred_masks[0])}')
            # test_process_2d_mask(predictions["instances"].pred_masks[0])
