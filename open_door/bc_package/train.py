...
'''
import torch
from PolicyNetwork import PolicyNetworkHighLow,gen_dataloader
...
def train():
    ...
    for epoch in range(N_EPOCHS):
        ...
        for data, labels_sq, labels_params in dataloader:  # data: # BATCH_SIZE * N_ENCODED_FEATURES * 1; labels_sq: # BATCH_SIZE * N_SEQUENCES; labels_params: BATCH_SIZE * N_SEQUENCES * N_PARAMS
            optimizer.zero_grad()
            high_level_output, high_level_sq, low_level_params = model(data)  # output: BATCH_SIZE * N_SEQUENCES * TYPES_SEQUENCES
            loss = 0
