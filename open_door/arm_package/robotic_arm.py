...
class Move_Plan():
    ...
    def Movej_P_Cmd(self, pose, v, trajectory_connect, r=0, block=True):
        ...
        # logger_.info(f'Movej_P_Cmd Execution Result:{tag}')
...
class Arm(Set_Joint, Get_Joint, Tcp_Config, Tool_Frame, Work_Frame, Arm_State, Initial_Pose, Move_Plan, Teaching,
    ...
    def print_frame(self,frame):
        print(f'====== frame info: ======')
        print(f'frame_name:{frame.frame_name.name}')
        print(f'frame_position:{frame.pose.position.x,frame.pose.position.y,frame.pose.position.z}')
        print(f'frame_euler:{frame.pose.euler.rx,frame.pose.euler.ry,frame.pose.euler.rz}')
        print(f'frame_quaternion:{frame.pose.quaternion.w,frame.pose.quaternion.x,frame.pose.quaternion.y,frame.pose.quaternion.z}')
        print(f'frame_payload:{frame.payload}')
        print(f'frame_cxcycz:{frame.x,frame.y,frame.z}')
        print(f'============')